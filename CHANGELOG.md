...
- 完成资料时间模型升级：schema v8→v11，附件获得可索引的 `timeline_at / timeline_end_at / time_precision / time_source / time_confidence / timezone_offset`，历史原始时间继续保留在加密 metadata 中；
- 增加旧资料时间索引后台回填，支持暂停/继续、失败隔离和重启后续跑，启动/登录不做全库回填；
- 增加年/月/日/小时轻量统计索引及增量维护，时间轴查询不再依赖全库 metadata 解密；
- 资料中心建立“年 / 月 / 日三层横轴 + 日内纵轴”的十字时间轴，支持时间段色带、资料数量、默认当前年、自动定位最近有资料日期；
- 日内资料采用轻量文件行、密集时间段自适应聚合、分页继续加载及上一/下一有资料日期导航；
- 新增本机自动扫描源：解锁后增量扫描、手动立即扫描、暂停/继续、普通/大型资料统一复用现有加密入库链路；
- 扫描源绝对路径加密保存，增量索引只保留必要文件身份信息；源文件缺失不会删除 LifeGraph 已归档副本；
- 人生图谱日期格直接读取结构化资料统计自动点亮，避免首页为资料状态逐条解密附件；
- 新增“时间待确认 / 修正时间”维护闭环，允许将低置信度资料手工确认到日期或秒级，并保留原始识别时间；
- 年/月/日右侧抽屉改为数据库时间索引先筛选、当前页再解密，资料区 12 项/批懒加载，消除大资料库抽屉全库解密瓶颈；
- 首页默认启动回归首页，人生/年/月三条进度压缩并合并到顶部主模块；
- `.lifevault` 保持 format v3，schema v11、扫描源和时间索引继续纳入核心备份恢复；
- 正式稳定版统一前端、FastAPI、Python 包和 `.lifevault` producer 版本为 `0.0.10`。
- 正式收口回归基线：264 项测试（37 个测试文件分批）通过，JavaScript 语法检查与 Python 编译检查通过。
